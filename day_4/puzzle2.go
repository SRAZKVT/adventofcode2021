...
func main() {
	file, err := os.Open("day_4/input2.txt")
	if err != nil {
		log.Fatal(err.Error())
	}
	...
	for len(allBoards) != 1 {
		...
		removeAllWinningBoards()
		pos++
	}
	for !isWinningBoard(allBoards[0]) {
		markAllBoards(numbersToCome[pos])
		pos++
	}

	lastNumber := numbersToCome[pos-1]
	losingBoard := allBoards[0]
	sumAllUnmarked := sumAllUnmarked(losingBoard)
	fmt.Println(lastNumber, "last")
	fmt.Println(sumAllUnmarked, "sum")
	fmt.Println(lastNumber * sumAllUnmarked)
}

func removeAllWinningBoards() {
